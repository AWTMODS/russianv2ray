...
const handlePayment = async (ctx, months, cost) => {
    try {
        ...
        // Auto-create user if missing (Migration Fix)
        if (!user) {
        ...
        // If user already has a UUID, maybe reuse? Ideally generate new for premium isolation
        // BUT 3X-UI allows multiple clients. best to generate new.
        ...
        if (result.success) {
            ...
            ctx.reply('❌ Ошибка активации на сервере. Обратитесь в поддержку: ' + result.msg);
        }
